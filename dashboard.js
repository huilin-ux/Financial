...
function renderDCA(){
  ...
  Object.entries(byTk).forEach(([tk,data])=>{
    ...
    // Historical entries: collapse into one summary row instead of listing each
    const histSummaryRow=historicalEntries.length>0?(()=>{
      const dates=historicalEntries.map(e=>e.date).sort();
      const from=dates[0]?.slice(0,7);const to=dates[dates.length-1]?.slice(0,7);
      const rangeStr=from===to?from:`${from} ～ ${to}`;
      return `<div style="padding:10px 0;border-bottom:1px solid rgba(26,37,53,0.5);opacity:0.55"><div style="font-family:var(--mono);font-size:13px;color:var(--text-dim)">📅 歷史紀錄 ${historicalEntries.length} 筆（${rangeStr}）</div><div style="font-family:var(--mono);font-size:11px;color:var(--text-dim);margin-top:3px">成交價不可考${meta.avgCost?' — 平均成本以券商 App 數字為準':''}</div></div>`;
    })():'';
    const nonHistEntries=entries.filter(e=>!e.historical||e.price);
    const entryRows=histSummaryRow+nonHistEntries.map(e=>{
